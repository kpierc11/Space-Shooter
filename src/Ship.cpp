...
Ship::Ship(Game *game)
	: Actor(game), mBullets({}), mAmmoCount(100), mShipRotation(1.0f)

{
	SpriteComponent *ship = new SpriteComponent(this, 30);
	SDL_Texture *text = this->GetGame()->LoadTexture("assets/PixelSpaceRage/128px/PlayerBlue_Frame_01_png_processed.png");
	...
	{ 
		...
		bullet->mState = BulletState::Active;
...
void Ship::FireBullet()
{
	if (mAmmoCount != 0)
	{
		mBullets[mAmmoCount]->mState = BulletState::Active;
	}
}
